...
class model_plots(object):
    ...
    def __init__(self, model, feature_data=[], label_data=[], description=[], seed=999):
    ...
    def dataprep_modevalplots(self):
        ...
        for i in range(0, len(self.description)):
            ...
            dataset = pd.concat([self.feature_data[i], probabilities, y_true], axis=1,
                                join_axes=[self.feature_data[i].index])
    ...
    def input_modevalplots(self):
        ...
        for i in self.model.classes_:
            for j in self.description:
                ...
                eval_t_agg.loc[
                    (np.ceil(eval_t_agg.pct_ref.astype(float) * 10) / 10.0) >= eval_t_agg.gain_ref, 'gain_opt'] = \
    ...
    def cumulative_lift_chart(self, colors=[], smallest_class=True):
        eval_t_tot = self.input_modevalplots()
        # define number of plots
        if smallest_class == True:
            # if smallest_class is True it wil find return the smallest class in the first prediction label set
            classes = [self.label_data[0].value_counts(ascending=True).idxmin()]
        else:
    ...
    def response_chart(self, colors=[], smallest_class=True):
        eval_t_tot = self.input_modevalplots()
        # define number of plots
        if smallest_class == True:
            # if smallest_class is True it wil find return the smallest class in the first prediction label set
    ...
    def cumulative_response_chart(self, colors=[], smallest_class=True):
        eval_t_tot = self.input_modevalplots()
        # define number of plots
        if smallest_class == True:
            # if smallest_class is True it wil find return the smallest class in the first prediction label set
    ...
    def cumulative_gains_chart(self, colors=[], smallest_class=True):
        eval_t_tot = self.input_modevalplots()
        # define number of plots
        if smallest_class == True:
            # if smallest_class is True it wil find return the smallest class in the first prediction label set
            ...
            for col, j in enumerate(self.description):
                for tv in self.model.classes_:
                    add_origin_add = pd.DataFrame(
                        {'modelset': [j], 'target_value': [tv], 'decile': [0], 'tot': [0], 'pos': [0], 'neg': [0],
                         'pct': [0],
                         'cumpos': [0], 'cumneg': [0], 'cumtot': [0], 'cumpct': [0], 'gain': [0], 'cumgain': [0],
                         'gain_ref': [0],
                    ...
                    eval_t_tot_origin = eval_t_tot_origin.set_index(keys=['target_value', 'modelset', 'decile'],
                                                                    drop=False)
                plt.plot(eval_t_tot_origin.decile[(eval_t_tot_origin.modelset == self.description[col]) & (
                            eval_t_tot_origin.target_value == i)], \
                         eval_t_tot_origin.gain_ref[(eval_t_tot_origin.modelset == self.description[col]) & (
                                     eval_t_tot_origin.target_value == i)], \
                         color='black', linestyle='dashed', label='no model')
                plt.plot(eval_t_tot_origin.decile[(eval_t_tot_origin.modelset == self.description[col]) & (
                            eval_t_tot_origin.target_value == i)], \
                         eval_t_tot_origin.gain_opt[(eval_t_tot_origin.modelset == self.description[col]) & (
                                     eval_t_tot_origin.target_value == i)], \
                         color='black', linestyle='dashed', label='optimal')
                plt.plot(
                    eval_t_tot_origin.decile[(eval_t_tot_origin.modelset == j) & (eval_t_tot_origin.target_value == i)], \
                    eval_t_tot_origin.cumgain[
                        (eval_t_tot_origin.modelset == j) & (eval_t_tot_origin.target_value == i)], \
                    color=colors[col], label='%sset' % j)
    ...
    def multiplot(self, colors=[], smallest_class=True):
        eval_t_tot = self.input_modevalplots()
        # define number of plots
        if smallest_class == True:
            # if smallest_class is True it wil find return the smallest class in the first prediction label set
        ...
        for i in classes:
            ...
            for col, j in enumerate(self.description):
                ...
                for tv in self.model.classes_:
                    ...
                    eval_t_tot_origin = eval_t_tot_origin.set_index(keys=['target_value', 'modelset', 'decile'],
                                                                    drop=False)
                ax1.plot(eval_t_tot_origin.decile[(eval_t_tot_origin.modelset == self.description[col]) & (
                            eval_t_tot_origin.target_value == i)], \
                         eval_t_tot_origin.gain_ref[(eval_t_tot_origin.modelset == self.description[col]) & (
                                     eval_t_tot_origin.target_value == i)], \
                         color='black', linestyle='dashed', label='no model')
